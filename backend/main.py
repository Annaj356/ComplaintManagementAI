...
# Allow your frontend to call this API — tighten origins in production
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# =========================
@app.post("/complaint", status_code=201)
def create_complaint(data: ComplaintRequest, db=Depends(get_db)):
    conn, cursor = db
    ...
    try:
        cursor.execute(
            """
            INSERT INTO complaints
            (user_id, title, description, category, priority, department, status)
    ...
    try:
        requests.post(
            "https://jefrymammen.app.n8n.cloud/webhook-test/complaint-email",
            json={
                "name": user["name"],
                "email": user["email"],
                "title": data.title,
...
# =========================
@app.get("/complaints")
def get_complaints(limit: int = 50, offset: int = 0, db=Depends(get_db)):
    conn, cursor = db
...
# =========================
@app.put("/complaint/{id}")
def update_status(id: int, data: StatusUpdateRequest, db=Depends(get_db)):
    conn, cursor = db
